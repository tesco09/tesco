...
export default function AdminNotification() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${BaseUrl}/notifications/receiver/Admin`);
    ...
    const allSeen = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${BaseUrl}/notifications/all-seen/Admin`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            {loading ? <LoadingSpinner /> : <div className="flex flex-col items-center w-full md:w-[75%] md:ml-[25%]">
